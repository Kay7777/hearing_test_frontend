...
class Page10Question extends React.Component {
  ...
  componentDidMount = async () => {
    const doc = await axios.get("/api/digits/post-test-1/questions");
    this.setState({ questions: doc.data });
  };

  handleDelete = async (id) => {
    await axios.delete("/api/digits/post-test-1/questions/" + id);
    await this.setState({ question: "" });
    this.componentDidMount();
  };

  handleAdd = async () => {
    await axios.post("/api/digits/post-test-1/questions", {
