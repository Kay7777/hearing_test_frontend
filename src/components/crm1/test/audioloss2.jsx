...
class TestMain extends React.Component {
    ...
    componentDidMount = async () => {
        for (var i = 1; i < 5; i++) {
            for (var j = 1; j < 9; j++) {
                const audio1 = new Audio(process.env.PUBLIC_URL + "/crm1-loss-audio/" + i.toString() + j.toString() + ".wav");
                audio1.volume = 0;
                try {
                    await audio1.play();
                    audio1.pause();
    ...
    playAudio = async () => {
        ...
        let sourceAudio = new Audio(process.env.PUBLIC_URL + "/crm1-loss-audio/" + question + ".wav");
