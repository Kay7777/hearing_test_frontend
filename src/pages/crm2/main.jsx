...
class Main extends React.Component {
  ...
  componentDidMount = async () => {
    const doc = await axios.get("/api/crm2/reversals");
    await this.setState({reversals: doc.data.reversals});
    const order = ["Source1", "Source2", "Source3", "Source4"];
    this.shuffle(order);
    this.setState({ order });
    setTimeout(() => {
  ...
  handleSubmit = async (callback) => {
    ...
    await axios.post("/api/crm2/user/data", {
  ...
  returnTrainingMode = (num, func) => {
    ...
    } else if (order[num] === "Source3") {
      return <Source3 reversals={reversals} volume={volume} handleClick={func} cycle={"Source"} block={num} />;
    } else if (order[num] === "Source4") {
      return <Source4 reversals={reversals} volume={volume} handleClick={func} cycle={"Source"} block={num} />;
    }
  }
